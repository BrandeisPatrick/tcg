import type { StatusId } from '@/engine/types';
import { palette, fonts } from './tokens';

const BUFFS: Set<StatusId> = new Set(['weapon_power','spirit_power','bullet_resist','spirit_resist','shield','unstoppable']);
const DEBUFFS: Set<StatusId> = new Set(['stun','silenced','disarm','sleep','bleed','vulnerable']);
// Statuses that are present on the engine state but render no visible chip — they
// communicate via card-specific visual cues (e.g., the invincibility aura) instead.
const HIDDEN: Set<StatusId> = new Set(['invincibility']);
// (utility set is the implicit "other" — long_range, bench_only, etc.)

function colorFor(id: StatusId): string {
  if (BUFFS.has(id)) return palette.status.buff;
  if (DEBUFFS.has(id)) return palette.status.debuff;
  return palette.status.utility;
}

// Short, readable labels for each status. Drives the chip text.
const STATUS_LABELS: Record<string, string> = {
  // Buffs
  weapon_power:  '+ATK',
  spirit_power:  '+SPI',
  bullet_resist: 'Bullet Res',
  spirit_resist: 'Spirit Res',
  shield:        'Shield',
  unstoppable:   'Unstop',
  // Debuffs
  stun:          'Stun',
  silenced:      'Silence',
  disarm:        'Disarm',
  sleep:         'Sleep',
  bleed:         'Bleed',
  vulnerable:    'Vuln',
  // Passives
  long_range:    'Range',
};

// Statuses where the value is a magnitude that should be shown next to the label.
// Stun/Silence/Disarm/Sleep/Vuln are binary — value=1, hide it.
const VALUE_STATUSES: Set<string> = new Set([
  'shield', 'bullet_resist', 'spirit_resist', 'weapon_power', 'spirit_power', 'bleed',
]);

interface Props {
  id: StatusId;
  value?: number;
  duration?: number;
  /** 'compact' = bench portrait, 'normal' = active portrait, 'large' = preview sheet. */
  size?: 'compact' | 'normal' | 'large';
}

export function StatusIcon({ id, value, duration, size = 'normal' }: Props) {
  if (HIDDEN.has(id)) return null;
  const color = colorFor(id);
  const label = STATUS_LABELS[id] ?? id;
  const showValue = typeof value === 'number' && (VALUE_STATUSES.has(id) || value > 1);
  const showDuration = typeof duration === 'number' && duration > 0 && duration < 99;

  const fontSize = size === 'compact' ? 8 : size === 'large' ? 13 : 9.5;
  const padV = size === 'compact' ? 2 : size === 'large' ? 6 : 3;
  const padH = size === 'compact' ? 5 : size === 'large' ? 12 : 6;

  return (
    <span
      title={`${label}${showValue ? ` ${value}` : ''}${showDuration ? ` (${duration} turns)` : ''}`}
      style={{
        display: 'inline-flex', alignItems: 'center', gap: 4,
        padding: `${padV}px ${padH}px`,
        background: `linear-gradient(180deg, ${color}, ${color}d0)`,
        color: '#fff',
        border: `1px solid rgba(0,0,0,0.45)`,
        borderRadius: 4,
        fontFamily: fonts.display,
        fontSize,
        fontWeight: 800,
        letterSpacing: '0.08em',
        textTransform: 'uppercase',
        textShadow: '0 1px 1px rgba(0,0,0,0.55)',
        boxShadow: `0 1px 3px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.18)`,
        whiteSpace: 'nowrap',
        lineHeight: 1.1,
      }}
    >
      <span>{label}</span>
      {showValue && (
        <span style={{
          padding: '0 4px',
          background: 'rgba(0,0,0,0.32)',
          borderRadius: 6,
          fontSize: fontSize - 0.5,
          letterSpacing: '0.04em',
          fontVariantNumeric: 'tabular-nums',
        }}>{value}</span>
      )}
      {showDuration && (
        <span style={{
          opacity: 0.78,
          fontSize: fontSize - 1.5,
          letterSpacing: '0.02em',
          fontVariantNumeric: 'tabular-nums',
        }}>({duration})</span>
      )}
    </span>
  );
}
